...
def stock_sheet_sources():
    ...
    for item in re.split(r"[\n,]+", configured):
        ...
        url = text
        if "|" in text:
            brand, url = [part.strip() for part in text.split("|", 1)]
        sources.append({"brand": brand, "url": url})
    return sources


def stock_field(row, *names):
...
def stock_model(row):
    return clean_text(stock_field(row, "model", "vehicle model", "car model", "model name")) or "Unknown"


def stock_color(row):
    return clean_text(stock_field(row, "color", "colour", "vehicle color", "vehicle colour")) or "Unknown"


def stock_booking_no(row):
    return stock_field(row, "booking no", "booking_no", "booking number", "booking id", "order no", "customer order no")


def stock_booking_type(row):
    text = norm(stock_field(row, "booking type", "booking mode", "booking source", "generated by", "system generated", "manual/system"))
    if "system" in text or "auto" in text:
        return "System Generated"
    if "manual" in text:
        return "Manual"
    return "Assigned - Type Missing" if stock_booking_no(row) else "Unassigned"


def stock_bucket(row):
    status = stock_status(row)
    booking_no = stock_booking_no(row)
    if "bbnd" in status:
        return "BBND"
    if "deliver" in status or "retail" in status or "sold" in status:
        return "Delivered"
    if "hold" in status or "blocked" in status:
        return "Hold"
    if "free" in status or "available" in status or (not status and not booking_no):
        return "Free"
    if booking_no:
        return "Hold"
    return "Free"
...
def fetch_stock_rows():
    rows = []
    issues = []
    for source in stock_sheet_sources():
        url = source["url"]
        try:
            with urlopen(url, timeout=12) as response:
                raw = response.read().decode("utf-8-sig", errors="replace")
        except Exception as exc:
            issues.append(f"Could not load stock sheet: {exc}")
            continue
        if "<html" in raw[:500].lower() or "<!doctype html" in raw[:500].lower():
            issues.append("Google Sheet is not publicly readable. Share it as Anyone with the link can view or publish CSV.")
            continue
        reader = csv.DictReader(io.StringIO(raw))
        for row in reader:
            if not any(clean_text(value) for value in row.values()):
                continue
            row["_sheet_brand"] = source["brand"]
            rows.append(row)
    return rows, sorted(set(issues))


def stock_brand_ok(row, selected_brand):
    brand = stock_field(row, "brand", "make", "company") or row.get("_sheet_brand", "")
    return not brand or norm(brand) == norm(selected_brand)


def stock_matches_filters(row, selected_brand, allowed_locations, filters):
    if not stock_brand_ok(row, selected_brand):
        return False
    if stock_location(row) not in set(allowed_locations):
        return False
    locations = {clean_location(item) for item in filter_values(filters, "location")}
    models = {norm(item) for item in filter_values(filters, "model")}
    colors = {norm(item) for item in filter_values(filters, "color")}
    if locations and stock_location(row) not in locations:
        return False
    if models and norm(stock_model(row)) not in models:
        return False
    if colors and norm(stock_color(row)) not in colors:
        return False
    return True


def summarize_stock(rows, key_func, label):
    ...
    for key in sorted(grouped):
        items = grouped[key]
        buckets = Counter(stock_bucket(item) for item in items)
        booking_types = Counter(stock_booking_type(item) for item in items if stock_booking_no(item))
        output.append([
            key,
            len(items),
            buckets["Delivered"],
            ...
            buckets["Free"],
            buckets["Hold"],
            sum(1 for item in items if stock_booking_no(item)),
            booking_types["System Generated"],
            booking_types["Manual"],
            booking_types["Assigned - Type Missing"],
        ])
    output.append([
        "Total" if label != "Model / Color" else "Total",
        sum(row[1] for row in output),
        sum(row[2] for row in output),
        sum(row[3] for row in output),
        sum(row[4] for row in output),
        sum(row[5] for row in output),
        sum(row[6] for row in output),
        sum(row[7] for row in output),
        sum(row[8] for row in output),
        sum(row[9] for row in output),
    ])
    return output


def build_stock_report(user, selected_brand, filters):
    raw_rows, issues = fetch_stock_rows()
    allowed_locations = user["locations"]
    rows = [row for row in raw_rows if stock_matches_filters(row, selected_brand, allowed_locations, filters)]
    available_rows = [row for row in raw_rows if stock_brand_ok(row, selected_brand) and stock_location(row) in set(allowed_locations)]
    options = {
        "locations": sorted({stock_location(row) for row in available_rows}),
        "sources": [],
        "models": sorted({stock_model(row) for row in available_rows if stock_model(row)}),
        "colors": sorted({stock_color(row) for row in available_rows if stock_color(row)}),
    }
    buckets = Counter(stock_bucket(row) for row in rows)
    assigned = [row for row in rows if stock_booking_no(row)]
    booking_types = Counter(stock_booking_type(row) for row in assigned)
    cards = [
        {"label": "TOTAL STOCK", "value": len(rows)},
        {"label": "DELIVERED", "value": buckets["Delivered"]},
        {"label": "BBND", "value": buckets["BBND"]},
        {"label": "FREE", "value": buckets["Free"]},
        {"label": "HOLD", "value": buckets["Hold"]},
        {"label": "ASSIGNED", "value": len(assigned)},
    ]
    headers = ["Group", "Total Stock", "Delivered", "BBND", "Free", "Hold", "Assigned", "System Generated", "Manual", "Type Missing"]
    tables = []
    if issues and not rows:
        tables.append({
            "title": "Stock Sheet Access",
            "wide": True,
            "headers": ["Issue", "Action Required"],
            "rows": [[issue, "Make the Google Sheet public/viewable or publish it as CSV, then redeploy/retry."] for issue in issues],
        })
    tables.extend([
        {"title": "Stock Summary by Location", "wide": True, "headers": ["Location"] + headers[1:], "rows": summarize_stock(rows, stock_location, "Location")},
        {"title": "Stock Summary by Model", "wide": True, "headers": ["Model"] + headers[1:], "rows": summarize_stock(rows, stock_model, "Model")},
        {"title": "Stock Summary by Color", "wide": True, "headers": ["Color"] + headers[1:], "rows": summarize_stock(rows, stock_color, "Color")},
        {
            "title": "Stock Summary by Model & Color",
            "wide": True,
            "headers": ["Model / Color"] + headers[1:],
            "rows": summarize_stock(rows, lambda row: f"{stock_model(row)} / {stock_color(row)}", "Model / Color"),
        },
        {
            "title": "Booking Assignment Type",
            "headers": ["Type", "Count"],
            "rows": [
                ["System Generated", booking_types["System Generated"]],
                ["Manual", booking_types["Manual"]],
                ["Assigned - Type Missing", booking_types["Assigned - Type Missing"]],
...
def build_report(user, selected_brand, filters=None, report_type="location"):
    filters = filters or {}
    report_type = report_type if report_type in {"location", "source", "model", "target", "incentive", "stock"} else "location"
    if selected_brand not in user["brands"]:
        raise ValueError("Brand is not allowed for this user")

    if report_type == "stock":
        return build_stock_report(user, selected_brand, filters)
